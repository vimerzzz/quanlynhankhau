...
create table loai_dong_gop(
  ...
  tenKhoanDongGop varchar(100) not null,
  khoanBatBuoc int(1) not null,
  khoanTuNguyen int(1) not null
);

create table nguoi_dong_gop(
  ID int auto_increment primary key not null,
  idHoKhauDongGop int not null,
  idNguoiDongGop int not null,
  idLoaiDongGop int not null,
  soTienDongGop int not null default 0,
  ngayNop date default current_timestamp
);